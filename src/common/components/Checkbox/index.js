...
const CheckBox = ({ className, isChecked, labelText, value, id, htmlFor, labelPosition, isMaterial, disabled, ...props }) => {
    // use toggle hooks
    ...
    return (
        <CheckBoxStyle className={addAllClasses.join(' ')} {...props}>
            <label htmlFor={htmlFor}>
                ...
                <input type="checkbox" className="checkbox" id={id} value={value} checked={toggleValue} onChange={toggleHandler} disabled={disabled} {...props} />
                <div />
            </label>
        </CheckBoxStyle>
    )
