...
const toolTipHandleFunc = (props) => {
    ...
    return (
        <Tooltip prefixCls="reusecore__rc_slider_tooltip rc-slider-tooltip" overlay={value} visible={dragging} placement="top" key={index}>
...
const SliderBox = (props) => {
    const { min, max, stepper, dots, slideDefaultValue, vertical, disabled, handleChangefunc, ...others } = props
    return (
        <Slider dots={dots} min={min} max={max} step={stepper} value={slideDefaultValue} onChange={handleChangefunc} disabled={disabled} handle={toolTipHandleFunc} vertical={vertical} {...others} />
    )
}

const RangeBox = (props) => {
    const { min, max, stepper, unit, dots, placement, rangeDefaultValue, vertical, disabled, handleChangefunc, ...others } = props
        ...
        <Fragment>
            <RangeBoxStyle className={addAllClasses.join(' ')}>
                <label>
                    ...
                    {type && type === 'range' ? <RangeBox {...props} /> : <SliderBox {...props} />}
                    <div>
                        <div />
                    </div>
                </label>
