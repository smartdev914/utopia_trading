...
import { TooltipStyle, TriggerStyle, BubbleSize, BubbleStyle } from './tooltip.style'

const Tooltip = ({ className, position, tooltipColor, bubbleSize, bubbleStyle, triggerStyle, message, children, ...props }) => {
    // tooltip local state
    ...
    return (
        <TooltipStyle className={addAllClasses.join(' ')} onMouseLeave={hideTooltip} tooltipColor={tooltipColor} {...props}>
            {state.open && (
                <BubbleSize className={`tooltip-bubble tooltip-${position}`} {...bubbleSize}>
                    <BubbleStyle className="tooltip-message" {...bubbleStyle}>
                        {message}
                    </BubbleStyle>
                </BubbleSize>
            )}
            <TriggerStyle className="tooltip-trigger" onMouseOver={showTooltip} {...triggerStyle}>
                {children}
            </TriggerStyle>
        </TooltipStyle>
    )
