...
const Button = ({ type, title, icon, disabled, iconPosition, onClick, loader, loaderColor, isMaterial, isLoading, className, ...props }) => {
    ...
    const buttonIcon = isLoading !== false ? <Fragment>{loader ? loader : <Loader loaderColor={loaderColor || '#30C56D'} />}</Fragment> : icon && <span className="btn-icon">{icon}</span>

    // set icon position
    const position = iconPosition || 'right'

    return (
        <ButtonStyle type={type} className={addAllClasses.join(' ')} icon={icon} disabled={disabled} icon-position={position} onClick={onClick} {...props}>
