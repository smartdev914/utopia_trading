...
const GlideCarousel = ({
    ...
    return (
        <GlideWrapper className={addAllClasses.join(' ')} id={carouselSelector || 'glide'}>
            ...
            {controls && (
                <ButtonControlWrapper className="glide__controls" data-glide-el="controls" {...buttonWrapperStyle}>
                    <ButtonWrapper {...prevWrapper} className="glide__prev--area" data-glide-dir="<" aria-label="prev">
                        {prevButton ? prevButton : <DefaultBtn>Prev</DefaultBtn>}
                    </ButtonWrapper>
                    <ButtonWrapper {...nextWrapper} className="glide__next--area" data-glide-dir=">" aria-label="next">
            ...
            {bullets && (
                <BulletControlWrapper className="glide__bullets" data-glide-el="controls[nav]" {...bulletWrapperStyle}>
                    <Fragment>
                        {totalBullets.map((index) => (
                            <BulletButton key={index} className="glide__bullet" data-glide-dir={`=${index}`} aria-label={`bullet${index + 1}`} {...bulletButtonStyle} />
                        ))}
                    </Fragment>
