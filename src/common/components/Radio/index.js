...
const Radio = ({ className, isChecked, labelText, value, id, htmlFor, isMaterial, labelPosition, disabled, ...props }) => {
    // use toggle hooks
    ...
    return (
        <RadioBoxStyle className={addAllClasses.join(' ')} {...props}>
            <label>
                ...
                <input type="radio" className="radio" id={id} value={value} checked={toggleValue} onChange={toggleHandler} disabled={disabled} {...props} />
                <div />
            </label>
        </RadioBoxStyle>
    )
