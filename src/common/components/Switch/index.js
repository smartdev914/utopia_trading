...
const Switch = ({ className, switchColor, isChecked, labelText, labelPosition, switchSize, isMaterial, barColor, onChange, onFocus, onBlur, handleOnChange, ...props }) => {
    // use toggle hooks
    ...
    return (
        <SwitchStyle className={addAllClasses.join(' ')} switchColor={switchColor} switchSize={switchSize} barColor={barColor} {...props}>
            <label>
                {position === 'left' || position === 'right' || position === 'top' ? LabelField : ''}

                <input checked={toggleValue} onChange={handleOnChange} onBlur={onBlur} onFocus={onFocus} className="switch" type="checkbox" value={toggleValue} />
                <div>
                    <div />
                </div>
