    ...
    return (
        <Fragment>
            <RcDrawer open={open} onClose={toggleHandler} className={addAllClasses.join(' ')} {...props}>
                <div className="reusecore-drawer__close" onClick={toggleHandler} style={closeButtonStyle} role="button" tabIndex="0" onKeyPress={() => {}}>
                    {closeButton}
                </div>
                {children}
            </RcDrawer>
            <div className="reusecore-drawer__handler" style={{ display: 'inline-block' }} onClick={toggleHandler} role="button" tabIndex="0" onKeyPress={() => {}}>
                {drawerHandler}
            </div>
        </Fragment>
    )
