...
const InputGroup = ({ className, inputType, placeholder, selectOptions, inputValue, inputOnChange, selectOnUpdate, selectedValue, currency }) => {
    ...
    return (
        <ComponentWrapper className={addAllClasses.join(' ')}>
            <Input type={inputType} value={inputValue} placeholder={placeholder} onChange={inputOnChange} required={true} aria-label="input" />
            <SelectWrapper className="select_wrapper">
                <CurrentOption className="current_option" onClick={handleDropdown}>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="14.994" height="10.993" viewBox="0 0 14.994 10.993">
                        <path
                ...
                <Dropdown className={`dropdown ${state.open ? 'active' : 'hide'}`} ref={dropdownRef}>
